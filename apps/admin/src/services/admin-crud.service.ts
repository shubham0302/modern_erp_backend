...
export class AdminCrudService {
  ...
  async delete(input: { actorId: string; id: string }): Promise<{ success: boolean }> {
    ...
    return this.dataSource.transaction(async (manager) => {
      ...
      const target = await adminRepo.findOne({ where: { id: input.id } });
      if (!target) throw rpcError(ErrorCode.ADMIN_NOT_FOUND);

      if (target.isSuperAdmin) {
        const activeSupers = await adminRepo.count({
          where: { isSuperAdmin: true, isActive: true, deletedAt: IsNull() },
        });
        if (activeSupers <= 1) throw rpcError(ErrorCode.SUPER_ADMIN_PROTECTED);
      }

      await adminRepo.softDelete({ id: target.id });
      await adminRepo.update({ id: target.id }, { isActive: false });
  ...
  async restore(input: { actorId: string; id: string }): Promise<Admin> {
    ...
    return this.dataSource.transaction(async (manager) => {
      ...
      await adminRepo.restore({ id: target.id });
      await adminRepo.update({ id: target.id }, { isActive: true });

      const restored = await adminRepo.findOneOrFail({ where: { id: target.id } });
