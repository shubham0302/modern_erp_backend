...
export class StaffCrudGrpcController {
  ...
  @GrpcMethod('StaffService', 'SetStaffActive')
  async setActive(data: StaffProto.SetStaffActiveRequest): Promise<StaffProto.StaffResponse> {
    const staff = await this.crud.setActive({ id: data.id, isActive: data.isActive });
    return { staff: toProtoStaff(staff) };
  }
