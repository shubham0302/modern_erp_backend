...
export class RoleGrpcController {
  ...
  async update(data: StaffProto.UpdateRoleRequest): Promise<StaffProto.RoleResponse> {
    const role = await this.svc.update({
      id: data.id,
      name: data.name,
      description: data.description,
    });
    return { role: toProtoRole(role) };
  }
  ...
  @GrpcMethod('StaffService', 'ReplaceRolePermissions')
  async replace(
    data: StaffProto.ReplaceRolePermissionsRequest,
  ): Promise<StaffProto.RolePermissionsResponse> {
    const permissions = await this.svc.replacePermissions({
      roleId: data.roleId,
      permissions: data.permissions,
    });
    return { permissions: toProtoPermissions(permissions) };
  }
}
